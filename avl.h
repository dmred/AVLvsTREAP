...
public:
	...
	inline bool empty() { return !_height(root); }

	inline unsigned int size() { return _height(root); }
};
...
void AVL_tree<T>::_remove(Node<T>* & node, const T& value)
{
	if (node)
	{
		//if (!node->parent) { delete node;  return; }
		if (node == root&& !node->right && !node->left) { node = nullptr;  return; }
		if (value < node->key)
